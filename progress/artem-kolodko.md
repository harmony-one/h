...
2025-11-12 Wed: hedging model: [dynamic hedge update](https://github.com/harmony-one/portfolio-manager/commit/69eb621e81f7fed23c61ba9ea7e6ae7b0f13416c) (short only direction)
...
2025-10-18 Sat (4.0h): started implementation of LP copy-trading bot, using codebase of our LP bot. [Added](https://github.com/harmony-one/portfolio-manager/pull/49) events listener for NFT positions manager contract using Base Websocket RPC.
...
Grid positions system, described in [this document](https://docs.google.com/document/d/1ofKwx2V15DGb5cAe2v6Q_fMZkMHYTz0JZKXjAr4cynw/edit?disco=AAABrENIIcA), will not increase the returns from my opinion because the main problem remains the same: central position (initially in range) will be swapped to cbBTC or USDC; impermanent loss will be lower in absolute terms compared to full 100 tick investment, but so the fees earned (from the nature of concentrated liquidity, because smaller positions on left and right remains out of range most of the time). The only optimization in grid system idea is reduced gas fees because you don't need to close the position out of range (but still need to adjust liquidity, following current price).
