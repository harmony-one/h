...
2026-05-19 Tue: Fixed a BT init parity bug in the goldilocks_with_hedge backtest engine and added a regression test. Local smoke is now GREEN: the regime gate engages on the Nov 2025 crash, hard-OFF holds on benign cycles, and gated perp orders ≤ nogate. This invalidated all earlier cloud results (silent crashes in every wallet-target cycle) and unblocked re-launching the v15 canary and the five spec batches.

2026-05-18 Mon: Diagnosed the v15 validation blocker — v14 continuous wallet hedge emits 0 perp orders on the production LP anchor across 8 cloud runs / 2 LP anchors, so the gate cannot be validated end-to-end. A reserve_fraction sweep on extra_bear_oos confirmed the issue is anchor-level, not sizing. Recorded verdict RETIRE on prod LP anchor with two paths forward: re-target v15 on the v14 LP anchor with caveat, or close the v15 PR without prod deploy.

---

2026-05-15 Fri: Opened the v15 wallet-hedge regime-gate effort — a code-driven bear/benign detector that turns the wallet hedge OFF on benign tapes and lets continuous run only on bear tapes. Authored the Phase 0 detector research memo (three threshold candidates), expanded the validation matrix from 3 to 5 windows × 7 runs (adding fresh benign and bear OOS), and set up the local smoke scaffolding with explicit acceptance criteria.

2026-05-14 Thu: Finished the v15 "regime gate" feature for the LP-hedger strategy and got it ready for validation. The gate is a safety switch that automatically turns the wallet hedge OFF in calm/bullish markets and ON only in sustained bearish ones, so we stop paying the hedge cost when it isn't earning anything. Pushed the detector logic, the switch itself, monitoring fields, and three backtest configurations covering different historical periods (a long mixed window, a bull window, and a recent bearish window). Synced with the Aaron on the run plan.

2026-05-13 Wed: Implemented the v15 switch behind a default-off flag, so nothing changes in production until we explicitly enable it. When the market looks calm or bullish, the strategy behaves as if the hedge feature didn't exist; when it looks bearish, the existing v14 hedging logic runs as before. Walked the team through how the detector plugs into the strategy and agreed on which values to surface in logs and metrics so we can clearly see, both live and in backtests, why the gate flipped.

2026-05-12 Tue: Locked down the rules of the v15 detector before writing any strategy code. The detector decides "bear vs. not bear" from two simple market signals — the 14-day price return and the 30-day drawdown from the recent peak — with hysteresis (harder to switch into bear than out of it) and a short delay so a couple of noisy data points can't flip the regime. Reviewed the signal choices and thresholds with the team on historical data, confirmed the rule clearly distinguishes the recent bearish period from the calm/bullish ones, and deliberately picked this transparent rule over a black-box machine-learning model so the decision stays auditable.

2026-05-11 Mon: Synced with the team on the v14 strategy and agreed to put it on hold. The conclusion: the v14 wallet-hedge feature only pays off in clearly bearish markets and actively hurts performance in calm or bullish ones, and lower trading fees alone wouldn't fix that. We aligned on the v15 direction — make the strategy itself decide when the hedge should be active, with no manual market call from an operator — and wrote up the plan covering the detector approach, the historical periods we'll validate it on, and the criteria it needs to meet before going live.
...
2026-04-07 Tue: Rapid iteration — shipped v5→v6→v7 in one day. v6 added NAV drawdown guard and cost_drag_adaptive bands, cutting perp orders 46% and improving return +2.9pp. v7 replaced binary DD exit with graduated LP scaling (proportional size reduction instead of full exit), added daily cost cap and wider delta bands. Best result: v7-D at -1.94%, but v7-C (-2.12%, hedge_ratio=0.95) chosen as baseline for robustness. Key finding: transaction costs still consume 50–80% of LP fees across all versions.

2026-04-06 Mon: Built v4–v5 — core wallet hedge features: dynamic and auto wallet hedge (hedges on-chain BTC balance automatically), volatility regime scaling (adjusts hedge size by vol regime), trend-based scaling, and cost drag adaptation. Added 308 unit tests. In parallel on main: refactored batch extractor to NAV CSV as primary data source with three-basis risk metrics (portfolio/deployed/maintenance).

---

2026-04-03 Fri: Established the iteration workflow for hedge strategy optimization: one branch per version, each containing a batch spec + backtest analysis report committed to the repo. Switched from short 1-2 month backtests to 10-month full-cycle runs (May 2025 – Mar 2026) covering both bull rally and bear crash — this required upgrading GCP machine type due to OOM issues on smaller instances. Starting from v3 baseline which had basic vol regime gating and trend filtering but no wallet hedge, no drawdown protection, and no cost management — returning -5.7% over the test period.
