2025-03-27 Thu: Analyzing Pendle protocol's reward system through protocol documentation, contract code, and on-chain transactions. Confirmed reward information is stored on-chain via PendleGauge contracts linked to specific markets. Executed and [documented test transactions](https://docs.google.com/spreadsheets/d/1bSxmde2KMA4ELrS1inC19jNDok2UZU5UE2ltHn_NVkA/edit?usp=sharing) for borrow strategy (Vicuna Protocol) and collateral yield strategy (Pendle/Euler), capturing performance metrics and transaction flows. Continuing investigation to definitively determine if reward calculation system is fully on-chain.
