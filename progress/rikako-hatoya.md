...
2026-04-21 Tue: Team sync in SF, editing of meeting notes. Look into details of recommendations and v13 work as listed [here](https://github.com/polymorpher/lp-hedger/blob/main/docs/analysis/batches/goldilocks_with_hedge_corrected_202604/final_production_recommendations.md).
...
2026-03-14 Sat: Work on deferred tasks on lp-hedger from PR (needs new PR due to bigger change). Also revisiting of `goldilocks_steady_lp` strategy.
...
2026-02-13 Fri: Ran backtest on 0x33d... (new target wallet chosen during team sync) for 1 week, backtest seems to crash when running for a wallet with many transactions/long duration due to OOM from timeline-tool. Switched to 3-days and exported output, results indicated target wallet pnl: +$13k meanwhile backtest: -$129k, looked into logs to investigate difference, cause seems to be the copy-bot logic for swappimg tokens to maintain ratio.

2026-02-12 Thu: Initially attempted making a fast scanning version of the script to scan all pool events in one pass instead of running per-wallet timelines but AERO attribution was too broad (counting claims from pre-existing positions), went back to working on the original full script, improving the rewards calculation accuracy and run-time of previous script to find the top profitable wallets, fixed bugs, and [ran](https://docs.google.com/spreadsheets/d/14bAoAmOc0nD9AFC0uWNfqIZtL3laYS-Cr46bqD8ot2c/edit?gid=1266333545#gid=1266333545) for [past 24 hours, blocks 42024724~42067924, top 10 wallets, average active duration 2-500 blocks]. Reduced run-time of full script by: 1. skipping `initial_state` in timeline-tool, which scanned all NFPM tokens owned by each wallet across all pools, wasting time on calculating positions outside of the input block range (ignore pre-existing state), 2. disabling `trace_filter` in timeline-tool, which was trying to map claims to specific token IDs via traces, we now lose the per-position breakdown but attribute the total AERO rewards to the wallet as a whole.

2026-02-11 Wed: Got [results](https://docs.google.com/spreadsheets/d/1Chr2XgXsLeeeaZ_DCtxXnEUTucttftS8du6BBRgA8sA/edit?gid=1602428293#gid=1602428293) from yesterday, noticed fixed capital under-utilization bug when running $820k initial capital backtest (the threshold-based swap logic designed for small capital was skipping swaps) so fixed to always swap to optimal token ratio, also identified AERO rewards overestimation where backtest bot's liquidity wasn't included in `total_staked` denominator so added bot's liquidity to account for self-dilution, re-ran fixed script again with $820k liquidity (output [here](https://docs.google.com/spreadsheets/d/1OSDQsWx_TWyZM9yIgpydhLZeJ1p_1JorvIZAngPbEGI/edit?gid=1283492311#gid=1283492311)), and investigated IL & AERO difference between target wallet (found out backtest is always exposed to inter-swap price jumps if using `pool.slot0()` approach to get pool state, looking into scaled amounts approach). Worked on finding top wallets with my previous script (`find_top_performers.py`), fixed bug where wallets with no activity in our pool was showing up (this was due to scanning NFPM events globally instead of filtering for specific pool), investigated how to reduce run-time as it takes a lot of time to run analysis for many wallets.

2026-02-10 Tue: In-person team sync with Li and Aaron. [Re-run](https://docs.google.com/spreadsheets/d/1douEiFtx5l6HW6oRftXxeYjX1qafaKe0vcciQh9_rqA/edit?gid=394884923#gid=394884923) of backtest with same initial deposit as target wallet ($880k~), realized it's in copy-mode not fixed capital mode, which means inifinite token supply in wallet is assumed and ignores need of swap, currently re-running for same initial capital as target wallet ($820k~), and also modified code structure to cache timeline data by `(walletAddress, blockRange)` as it was previously only cached in folders sorted by run, which now means script will correctly read cached data and reduce time latency.

2026-02-09 Mon: Worked on Backtest: Fixed AERO rewards overestimation by implementing `calculate_bot_rewards_with_delay()` to account for when a whale unstakes, fixed price fallback issue, fixed swap formula bug with inverted decimal scaling that was causing bot to run out of money, fixed issue with negative token amount withdrawal caused when `sqrt_price` was exactly at a tick boundary by adding safeguards, and fixed issue with `optimize_capital_for_position`, which was showing a non-zero tick range even when bot's capital was fully depleted, by adding a check. Reduced runtime of backtest, which previously took 17+ hours for a 3.8-day run to be completed in 80 minutes end-to-end by the following optimizations: 1. derive block timestamps approximating with 2-second intervals instead of fetching each with `eth_getBlockByNumber`, 2. reward rate only changes per epoch so switched to one reward rate per epoch, 3. implemented parallel processing of earnings calculation (`gauge.earned()` and `gauge.stakedLiquidity()`), ran backtest with same initial deposit as real-bot ($826) output [here](https://docs.google.com/spreadsheets/d/1Tzki4pRXiiSxNWctj56dq2V04cJh6UjDgH_azLm_vxU/edit?pli=1&gid=1555461379#gid=1555461379).

---

2025-02-08 Sun: Switched backtest to running in background as process kept repeatedly dying from terminal timeout, looked into backtest crashes, which was caused by capital depletion and implemented fixes to safeguard logic, and fixed new bugs that were introduced when fixing capital depletion issue. Ran 1-week (41427727~41730126) backtest for new target-wallet `0xebe`, output [here](https://docs.google.com/spreadsheets/d/1UQjr0cXsI9w4oXwqKjDOjODlyg2eELcBvsIVPZ41m8Y/edit?gid=1592985986#gid=1592985986).

2025-02-07 Sat: Examined 1-week backtest results from Friday and noticed backtest having significantly more AERO, varying deposit amounts, and a much bigger IL. Conducted many back-and-forth analysis of the comparison results by running shorter test ranges (3 hours) to debug faster.

2026-02-06 Fri: Looked into how swap fees work, calculated average dynamic swap fee rate over Nov/Dec 2025 using data from database, and added swap fee tracking pipeline to backtest. Modified backtest to calculate swap fees using average rate (0.036%), updated timeline-tool to detect same transaction swaps and query `pool.fee()`, and extended analysis scripts to display swap fee in the output CSVs.
...
2026-01-16 Fri: Looking into how to update latest data with database. Look into GCP.
...
2025-12-24 ~ 2025-12-26: Christmas, no team sync.
...
2025-12-09 Tue: Still had issue at tick boundaries causing incorrect IL calculation, fixed issue and also created a script to display IL and hedge pnl per position. Ran analysis for August-October, output [here](https://drive.google.com/drive/folders/1eFJfyllcExzKarEF5tw0q0a-mxTt_QUC).
...
2025-11-28 Fri: Thanksgiving Weekend

2025-11-27 Thu: Thanksgiving Weekend

2025-11-26 Wed: Started looking into hedging repo. Noticed issue with hodl APR calculation regarding active time, fixed issue, ran timeline script for [0-5500 blocks median active time wallets](https://docs.google.com/spreadsheets/d/1av4wdCqZHGCIZlOiCua_PUkGIB20E0XBjxmIul9NCvI/edit?gid=262415682#gid=262415682) for 11/18/2025~11/25/2025, output analysis [here](https://docs.google.com/spreadsheets/d/1DKxxpFN3B3WLvdqtOVZPOcu2alWlez3kt84UtdzQV8c/edit?gid=1348002226#gid=1348002226).
...
2025-11-21 Fri: Re-run of timeline script for all wallets with more lenient bounds on the median active duration time (1-1000 blocks). Got apx. 50 new candidate wallets to later run on returns script to get performance analysis, 5 wallets have extremely high activity (10,000+ positions) taking extremely long to run timeline script, working on modifying code to reduce rpc call time.
...
2025-11-12 Wed: [Renewed](https://github.com/rikaa15/lp-returns/commit/f7227dc128fad566ff39c8c92b90e9a86c1c2bcf) returns script, made it more replicable and easier for batch analysis across many wallets by updating the directory structure and simpler run commands, added more analysis metrics (especially, added 'ratio' to compare copy-bot vs. target performance by ratio of capital deployed), fixed calculation for wallet's operating time. Ran analysis for Yuriy's newly deployed [bot](https://basescan.org/address/0x222b2bcb5c3ba4a932900a0ce7fe5ee400bd2985) which has new swap settings to preserves liquidity (all data [here](https://drive.google.com/drive/folders/1sPc1o5aWP0qy3tCCXW4PH7DvnBcsLG1-?usp=drive_link) and analysis [here](https://docs.google.com/spreadsheets/d/1iGnLQOUP1oku3PkSFr_lzqazsh6UPvjGvm2s0PS2nsQ/edit?hl=en&gid=1503134257#gid=1503134257)).

2025-11-11 Tue (Veteran's Day, 6.0h): Team sync, went over the target vs. copy bot [comparison analysis sheet](https://docs.google.com/spreadsheets/d/1_hH6EJYtdHDsJQCMLWKneiBvHHo7WMCV81-ogmu_r4k/edit?gid=659341843#gid=659341843) together (data: 11/10, 2hrs), and looked through the transactions (noticed strange bot behavior, ~10 positions with 0 active seconds, basically minting and withdrawing at the same time). [Modified](https://github.com/rikaa15/lp-returns/commit/f6d6d116b8c054dd9b7182da83621a35a50b2a00) returns script to not only compare copy vs. target bot but also batch process analysis for multiple wallets and compare their returns, ran it for other top wallet candidates (3/7 wallets were found to have activity during a chosen 2hr period), data [here](https://drive.google.com/drive/folders/13i5tkXV7ZdqA0i5tGNKFkR03KA3FKjqo).
...
2025-10-23 Thu: Re-did [analysis](https://docs.google.com/spreadsheets/d/1l94LzbwKTQCoN_JQTYBjcstW6XrTvC3NTE2n4YcQ0fk/edit?gid=716436460#gid=716436460) of wallet (0xa697a5b929CcA9b3F8a78c4E4Af1848506C06022) after Aaron's updated timeline script, raw data now matches data from Artem's portfolio-manager. Created Python script based on Theo's [LP HODL methodology](https://docs.google.com/document/d/1HLNhV_YXcNAoz3ynwu4u8YhwTyXEZ7FkK0jm1WXRht0/edit?tab=t.0#heading=h.31xptq69rgvj) (will clean and push repo tomorrow) and calculated XIRR: 136.38% (compare to simple XIRR: 99.54%). 

2025-10-22 Wed: Looked through each transaction for 0xa697a5b929CcA9b3F8a78c4E4Af1848506C06022 from timeline-tool output, ran cbTC price scraper, and checked rewards calculation. Investigating discrepancy between output from portfolio-manager.

2025-10-21 Tue: Started analysis on top wallet by adding running timeline script ([output here](https://docs.google.com/spreadsheets/d/1ltu_JxP8fLplMUhg7hVYILRAzMtTKkQKj-HdBML0q-Y/edit?gid=1673240954#gid=1673240954)), getting cbBTC prices (output [here](https://docs.google.com/spreadsheets/d/1WiATQRRoMNXPZt7QNs-CS2ycJFPe40c1dmw60SyoagE/edit?gid=520818490#gid=520818490)) and calculating returns [here](https://docs.google.com/spreadsheets/d/1l94LzbwKTQCoN_JQTYBjcstW6XrTvC3NTE2n4YcQ0fk/edit?gid=1498157474#gid=1498157474). Update [basescan token price script](https://github.com/rikaa15/basescan-price) to handle other actions besides `mint`+`burn` (`collect`, `gauge_deposit`, `gauge_withdraw`, `gauge_getreward`), and also take in actions csv file output from timeline script directly.
